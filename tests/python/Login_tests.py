...
# Hand built Login packet
# TP02<32><1><><>username\0<>password\0
# BTW These numbers \<> are in octal

u = "username"
s = "password"

header = "TP02" +  "\0\0\0\0" +  "\0\0\0\4" + "\0\0\0\32" 
data = "\0\0\0\11" + u + "\0" + "\0\0\0\11" + s + "\0"
...
def test(p):
	# Check the class
	if not isinstance(p, objects.Login):
...
def test1():
	"""\
	Tests if Login can be built using Header(header) and process(data)
	"""
...
def test2():
	"""\
	Tests if Login can be built using Header(header+data)
	"""
	try:
