...
class ServiceBrowser(object):
	...
	def __init__(self, zeroconf, type, listener):
		...
		self.engineTime = currentTimeMillis()
		...
		for type in self.type:
			self.zeroconf.addListener(self, DNSQuestion(type, _TYPE_PTR, _CLASS_IN))

	def updateRecord(self, zeroconf, now, record):
		"""Callback invoked by Zeroconf when new information arrives.

		Updates information required by browser in the Zeroconf cache."""
		if record.type == _TYPE_PTR and record.name in self.type:
	...
	def run(self, force=False):
		while not globals()['_GLOBAL_DONE']:
			...
			if self.engineTime <= now or force:
				self.engineTime = self.zeroconf.run()

			if self.nextTime <= now or force:
				for type in self.type:
					out = DNSOutgoing(_FLAGS_QR_QUERY)
					out.addQuestion(DNSQuestion(type, _TYPE_PTR, _CLASS_IN))
...
class Zeroconf(object):
	...
	def getServiceInfo(self, type, name, timeout=5000):
